...
The **color** prop allows you to specify the color of the button. The possible values for this prop are: primary, secondary, success, danger, warning and info. If variant is set to outline and text, the color prop will be ignored for not pass in.
...
The **duration** prop allows you to specify the duration of the animation that is used to transition the button from one state to another. The possible values for this prop are: short, medium, and long.
...
💡 Use the duration prop to specify the duration of the animation that is used to transition the button from one state to another.