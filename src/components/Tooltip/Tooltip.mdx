...
<Canvas>
  <Story of={TooltipStories.WithOutsideClick} />
</Canvas>

### Persistent Tooltip

This tooltip will remain open even when other tooltips are opened.

<Canvas>
  <Story of={TooltipStories.Persistent} />
</Canvas>

### Multiple Tooltips

Demonstrates different tooltip behaviors when multiple tooltips are present.

<Canvas>
  <Story of={TooltipStories.MultipleTooltips} />
</Canvas>
...
The **persistent** prop allows you to create tooltips that remain open even when other tooltips are opened. This is useful for tooltips that contain important information that should stay visible while the user interacts with other parts of the interface.
...
💡 Use the **outsideClick** prop when you want tooltips to close when users click elsewhere. This is particularly useful for click-triggered tooltips that contain interactive content.

💡 Use the **persistent** prop for important tooltips that should remain visible while users interact with other parts of the interface. However, use this sparingly as too many persistent tooltips can clutter the interface.

💡 When using multiple tooltips, consider the interaction between **persistent** and non-persistent tooltips. Non-persistent tooltips will close each other, but persistent tooltips will remain open.
